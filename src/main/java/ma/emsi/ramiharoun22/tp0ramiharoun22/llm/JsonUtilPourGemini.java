...
public class JsonUtilPourGemini implements Serializable {
    ...
    private String creerRequeteJson(String systemRole, String question) {
        String safeRole = (systemRole == null || systemRole.isBlank())
                ? "You are a helpful assistant. Answer clearly and concisely."
                : systemRole;

        String safeQuestion = (question == null) ? "" : question;

        // Ajout du rôle système et de la question dans le même "content"
        JsonArray parts = Json.createArrayBuilder()
                .add(Json.createObjectBuilder().add("text", safeRole))
                .add(Json.createObjectBuilder().add("text", safeQuestion))
                .build();
